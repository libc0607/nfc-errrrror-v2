...
m = Map("guest-wifi-ntagmod", translate("Guest-wifi-ntagmod"))

s = m:section(TypedSection, "guest-wifi-ntagmod", translate("Config"), translate("You can set guest wifi here. The wifi will be disconnected when enabling/disabling. When modifying the password, first disable the guest wifi, and then do the modification, save and apply. Finally check both Enable and Create, save and apply."))
s.anonymous = true 
s.addremove = false
...
use_ntag = s:option(Flag, "use_ntag", translate("Use NFC Tag (EXPERIMENTAL)"), translate("NTag hardware needed. See Github: libc0607/nfc-errrrror-v2 for more details."))
use_ntag.default = false
use_ntag.optional = false
use_ntag.rmempty = false
